...
exports.createGuestBooking = async (req, res) => {
  ...
  if (!playstationId || !bookingDate || !startTime || !durationHours) {
    return res
      ...
      .json({
        message:
          "Semua detail booking, termasuk metode pembayaran, harus diisi.",
      });
  }
  ...
  try {
    ...
    if (paymentMethod === "on_site") {
      res.status(201).json({
        message:
          "Permintaan booking Anda telah diterima. Mohon tunggu konfirmasi dari admin kami.",
        bookingId: bookingId,
...
exports.uploadPaymentProof = async (req, res) => {
  ...
  if (!req.file) {
    return res
      .status(400)
      .json({ message: "File bukti pembayaran diperlukan." });
  }
  ...
  try {
    ...
    res.status(200).json({
      message:
        "Bukti pembayaran berhasil diupload. Admin akan memverifikasi pembayaran Anda.",
    });
  } catch (error) {
