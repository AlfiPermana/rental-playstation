...
exports.getAvailableSlots = async (req, res) => {
    ...
    if (!playstationId || !date) {
        return res
            .status(400)
            .json({ message: "PlayStation ID and date are required." });
    }
    try {
...
exports.createBooking = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            message: "Server error saat memproses booking atau pembayaran. Silakan periksa log server.",
        });
    }
};
...
exports.uploadPaymentProof = async (req, res) => {
    ...
    if (!bookingId || !file) {
        ...
        if (file && file.path) {
            try { await fs.unlink(file.path); } catch (e) { console.error('Error deleting file:', e); }
        }
        return res.status(400).json({ message: 'Booking ID dan file bukti pembayaran harus disediakan.' });
    }
    ...
    try {
        ...
        if (booking.user_id !== userId) {
            ...
            return res.status(403).json({ message: 'Anda tidak diizinkan mengupload bukti untuk booking ini.' });
        }
        ...
        if (booking.payment_status !== 'pending') {
            ...
            if (file && file.path) {
             ...
             return res.status(400).json({ message: 'Bukti pembayaran hanya bisa diupload untuk booking yang statusnya pending.' });
        }
        ...
        res.status(200).json({ message: "Bukti pembayaran berhasil diupload!", proofUrl: proofUrl });
        ...
        if (file && file.path) {
            try {
                ...
                console.error("Error deleting uploaded file due to another error:", unlinkError);
            }
        }
