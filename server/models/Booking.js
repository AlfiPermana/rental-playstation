...
class Booking {
  ...
  static async create(
    ...
    const bookingStatus = paymentMethod === "on_site" ? "pending_confirmation" : "pending";
    const paymentStatus = paymentMethod === "on_site" ? "on_site" : "pending";
    ...
    try {
      ...
      const [result] = await db.execute(
        ...
        [
          ...
          durationHours,
          finalAmount, // Use final amount instead of base amount
          bookingStatus, // Gunakan status dinamis
          paymentStatus,
          customerName,
          customerEmail,
          customerPhone,
  ...
  static calculateFinalAmount(baseAmount, uniqueCode) {
    ...
    console.log(
      `calculateFinalAmount: ${baseAmount} + ${uniqueCode} = ${finalAmount}`
    );
    return finalAmount;
  }
  ...
  static async getAvailableSlots(playstationId, date) {
    ...
    const [existingBookings] = await db.execute(
      'SELECT start_time, duration_hours FROM bookings WHERE playstation_id = ? AND DATE(booking_date) = ? AND (status = "pending" OR status = "confirmed" OR status = "pending_confirmation")',
      [playstationId, selectedDate] // Tambahkan 'pending_confirmation' agar slotnya terblokir
    );
    ...
    for (let hour = openHour; hour < closeHour; hour++) {
      ...
      if (selectedDate === now.toISOString().split("T")[0]) {
        if (
          ...
          // Jika jam slot sudah lewat atau sama dengan jam sekarang (menit berapapun)
      ...
      for (const booked of existingBookings) {
        ...
        const bookedStartDateTime = new Date(
          `${selectedDate}T${booked.start_time}`
        );
        const bookedEndDateTime = new Date(
          bookedStartDateTime.getTime() + booked.duration_hours * 60 * 60 * 1000
        );

        const possibleSlotStartDateTime = new Date(
          `${selectedDate}T${slotStart}`
        );
        ...
        if (
          possibleSlotStartDateTime < bookedEndDateTime &&
          possibleSlotEndDateTime > bookedStartDateTime
        ) {
          isBooked = true;
          break;
        }
  ...
  static async confirmPaymentByAdmin(bookingId) {
    const [result] = await db.execute(
      "UPDATE bookings SET status = ?, payment_status = ? WHERE id = ? AND (payment_status = ? OR payment_status = ? OR payment_status = ?)",
      ["confirmed", "paid", bookingId, "pending", "uploaded","on_site"]
    );
    return result;
  }
