document.addEventListener("DOMContentLoaded", () => {
  ...
  const fetchPlaystations = async () => {
    try {
      ...
      playstationsData = await response.json(); // Simpan data lengkap
      if (response.ok) {
        playstationSelect.innerHTML =
          '<option value="">-- Pilih PlayStation --</option>';
        playstationsData.forEach((ps) => {
          if (ps.status === "available") {
            ...
            option.dataset.price = ps.price_per_hour; // Simpan harga di data-attribute
  ...
  const fetchAvailableSlots = async () => {
    ...
    calculateAndDisplayPrice(); // Reset harga saat pilihan berubah
    availableSlotsDiv.innerHTML =
      '<div class="slots-placeholder"><p>Memuat slot...</p></div>';

    if (!playstationId || !date || isNaN(duration) || duration < 1) {
      availableSlotsDiv.innerHTML =
        '<div class="slots-placeholder"><p>Pilih PS, tanggal, dan durasi.</p></div>';
      return;
    }

    try {
      const response = await fetch(
        `/api/guest/available-slots?playstationId=${playstationId}&date=${date}`
      );
  ...
  availableSlotsDiv.addEventListener("click", (e) => {
    if (e.target.classList.contains("available")) {
      document
        .querySelectorAll(".slot-item.selected")
        .forEach((el) => el.classList.remove("selected"));
      e.target.classList.add("selected");
      selectedSlotTime = e.target.dataset.startTime;
      calculateAndDisplayPrice(); // Hitung harga saat slot dipilih
      updateConfirmButtonStatus();
    }
  });
  // Event listener untuk perubahan form
  playstationSelect.addEventListener("change", () => {
    const selectedId = playstationSelect.value;
    selectedPlaystation =
      playstationsData.find((ps) => ps.id == selectedId) || null;
    fetchAvailableSlots();
    ...
    try {
      ...
      if (response.ok) {
        ...
        availableSlotsDiv.innerHTML =
          '<div class="slots-placeholder"><p>Pilih PS, tanggal, dan durasi.</p></div>';
        selectedSlotTime = null;
        calculateAndDisplayPrice(); // Sembunyikan harga setelah reset
        if (result.redirectUrl) {
          alert(
            `Booking berhasil dibuat. Berikan link ini ke pelanggan untuk pembayaran: ${result.redirectUrl}`
          );
        }
